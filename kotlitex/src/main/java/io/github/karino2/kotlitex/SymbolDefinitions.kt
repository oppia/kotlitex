...
// mode, but among our fonts, only Main-Italic defines this character "163".
    ...
    fun defineNormal() {
        ...
        for (ch in mathTextSymbols) {
            ...
            defineSymbol(math, main, textord, chstr, chstr)
        }

        // All of these are textords in text mode
        val textSymbols = "0123456789!@*()-=+[]<>|\";:?/.,"
        for (cha in textSymbols) {
            val ch = cha.toString()
    ...
    fun pair2String(first: Int, second: Int) = String(charArrayOf(first.toChar(), second.toChar()))
    ...
    fun defineWide() {
        ...
        for ((i, cha) in letters.withIndex()) {
            ...
            if (i < 26) {
                ...
                wideChar = pair2String(0xD835, 0xDD38 + i) // A-Z double struck
                defineSymbol(math, main, mathord, ch, wideChar)
                defineSymbol(text, main, textord, ch, wideChar)

                wideChar = pair2String(0xD835, 0xDC9C + i) // A-Z script
                defineSymbol(math, main, mathord, ch, wideChar)
                defineSymbol(text, main, textord, ch, wideChar)
            }
